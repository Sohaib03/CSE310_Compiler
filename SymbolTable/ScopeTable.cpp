...
bool ScopeTable::insert(string name, string type) {
    ...
    cout << "Inserted in ScopeTable# " << getId() << " at position " << hash_value << ", " << cnt << endl;
    return true;
}
...
SymbolInfo* ScopeTable::lookUp(string name) {
    ...
    while(sym_info != nullptr) {
        if (sym_info->getName() == name) {
            cout << "Found in ScopeTable# " << getId() << " at position " << hash_value << ", " << cnt << endl;
            break;
        }
...
bool ScopeTable::delete_key(string name) {
    ...
    if (sym_info == nullptr) return false;
        ...
        delete sym_info;
        cout << "Deleted entry at "  << hash_value << "," << cnt << " in the current scopetable" << endl;
        return true;
    } else {
